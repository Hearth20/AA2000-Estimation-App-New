...
import { computeBurglarAlarmRuleQuantities } from '../utils/materialQuantityRules';
...
const BurglarAlarmSurvey: React.FC<Props> = ({ onBack, onComplete, onNewFloorPlan, initialData, projectBuildingInfo }) => {
  ...
  useEffect(() => {
    ...
    if (!hasUploadedPlan || editingSensorIndex !== null) return;
    const isSensorDraftEmpty =
      !currentSensor.location &&
      !currentSensor.type &&
      !currentSensor.connection &&
      !currentSensor.riskLevel &&
      (!currentSensor.intrusionConcern || currentSensor.intrusionConcern.length === 0) &&
      !currentSensor.environment &&
      (!currentSensor.obstructions || currentSensor.obstructions.length === 0) &&
      !currentSensor.wallType;
    if (!isSensorDraftEmpty) return;
    const used = new Set(data.sensors.map((s) => (s.location || '').trim().toLowerCase()).filter(Boolean));
    const nextRoom =
      data.measurements?.rooms?.find((r: any) => !used.has((r?.name || '').trim().toLowerCase())) ||
      data.measurements?.rooms?.[data.sensors.length];
    const location = (nextRoom?.name || '').trim() || `Sensor ${data.sensors.length + 1}`;
    setCurrentSensor((prev) => ({
      ...prev,
      location: prev.location || location,
      environment: prev.environment || 'Indoor',
      type: prev.type || 'PIR',
      connection: prev.connection || 'Wired',
      wallType: prev.wallType || 'Concrete',
      riskLevel: prev.riskLevel || 'Medium',
      intrusionConcern: (prev.intrusionConcern && prev.intrusionConcern.length > 0) ? prev.intrusionConcern : ['Door'],
      obstructions: (prev.obstructions && prev.obstructions.length > 0) ? prev.obstructions : ['Furniture'],
      count: prev.count && prev.count > 0 ? prev.count : 1,
    }));
  }, [data.measurements, data.sensors, editingSensorIndex, currentSensor]);
  ...
  const autoFillBurglarFromRules = () => {
    if (!data.measurements) return;
    if (data.sensors.length > 0) return; // avoid overwriting

    const quantities = computeBurglarAlarmRuleQuantities({
      baData: data,
      measurements: data.measurements,
    });

    const pirCount = Math.max(0, Math.floor(quantities.pirSensorsCount));
    if (pirCount <= 0) return;

    const roomName = (data.measurements.rooms?.[0]?.name || '').trim() || 'Perimeter / Room Areas';

    const pirSensor: BurglarAlarmSensor = {
      id: Math.random().toString(36).substr(2, 9),
      location: roomName,
      riskLevel: 'Medium',
      intrusionConcern: ['Door'],
      environment: 'Indoor',
      type: 'PIR',
    ...
    setData((prev) => ({
      ...
      notification: {
        ...prev.notification,
        sirenIndoor: Math.max(0, quantities.sirenIndoorQty),
        sirenOutdoor: Math.max(0, quantities.sirenOutdoorQty),
      },
      controlPanel: {
        ...prev.controlPanel,
        keypads: Math.max(1, quantities.keypadsQtyRecommended),
      },
    }));
  };
                  ...
                  onChange={(m) =>
                    setData(prev => {
                      ...
                      return {
                        ...
                        notification: {
                          ...prev.notification,
                          sirenIndoor: Number(prev.notification.sirenIndoor) > 0 ? prev.notification.sirenIndoor : 1,
                          sirenOutdoor: Number(prev.notification.sirenOutdoor) > 0 ? prev.notification.sirenOutdoor : 1,
                          strobeLight: prev.notification.strobeLight === undefined ? true : prev.notification.strobeLight,
                        },
                        controlPanel: {
                          ...prev.controlPanel,
                          location: prev.controlPanel.location || (firstRoom ? `${firstRoom} - Control Panel` : 'Security Office'),
                          systemType: prev.controlPanel.systemType || 'Hybrid',
                          keypads: Number(prev.controlPanel.keypads) > 0 ? prev.controlPanel.keypads : 1,
                          simCardRequired: prev.controlPanel.simCardRequired === undefined ? true : prev.controlPanel.simCardRequired,
                          internetRequired: prev.controlPanel.internetRequired === undefined ? true : prev.controlPanel.internetRequired,
                          sirenLocation: prev.controlPanel.sirenLocation || 'Main Entrance',
                          sirenTypeRequired: prev.controlPanel.sirenTypeRequired || 'Internal',
                          monitoringType: prev.controlPanel.monitoringType || 'Self-Monitoring',
                          notificationMethod:
                            prev.controlPanel.notificationMethod && prev.controlPanel.notificationMethod.length > 0
                              ? prev.controlPanel.notificationMethod
                              : ['SMS', 'App'],
                          petsPresent: prev.controlPanel.petsPresent === undefined ? false : prev.controlPanel.petsPresent,
                          powerSourceAvailable:
                            prev.controlPanel.powerSourceAvailable === undefined ? true : prev.controlPanel.powerSourceAvailable,
                          cableRoutingPath: prev.controlPanel.cableRoutingPath || 'Ceiling',
                          estimatedCableLength:
                            prev.controlPanel.estimatedCableLength !== undefined
                              ? prev.controlPanel.estimatedCableLength
                              : 30,
                        },
                      };
                    })
        ...
        {step === 'SENSORS' && (
          <div className="animate-fade-in space-y-6">
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black text-blue-900 uppercase tracking-tight">Add Burglar Alarm Entry</h3>
              </div>
              <div>
            ...
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-6">
              ...
              {data.sensors.length === 0 ? (
                <div className="p-10 border-2 border-dashed border-slate-200 rounded-3xl text-center text-slate-400 font-bold uppercase tracking-widest text-[10px] space-y-4">
                  <div>No burglar alarm units mapped yet.</div>
                  <button
                    type="button"
                    disabled={!data.measurements}
                    onClick={autoFillBurglarFromRules}
