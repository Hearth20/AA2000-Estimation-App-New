...
import { computeAccessControlRuleQuantities } from '../utils/materialQuantityRules';
...
const AccessControlSurvey: React.FC<Props> = ({ onBack, onComplete, onNewFloorPlan, initialData, projectBuildingInfo }) => {
  ...
  const autoFillAccessFromRules = () => {
    if (data.doors.length === 0) return;
    const quantities = computeAccessControlRuleQuantities({
      acData: data,
    ...
    setData((prev) => ({
      ...
      controller: {
        ...prev.controller,
        // The BUILDING-step FloorPlan auto-fill sets `estimatedCableLength` to 30 as a generic fallback.
        // If the user hasn't provided a meaningful value yet, replace it with the rule-based estimate.
        estimatedCableLength:
          prev.controller.estimatedCableLength === undefined ||
          prev.controller.estimatedCableLength === null ||
          prev.controller.estimatedCableLength === 30
            ? Math.max(0, Math.round(quantities.estimatedCableLengthRecommendedMeters))
            : prev.controller.estimatedCableLength,
        upsRequired:
          prev.controller.upsRequired !== undefined && prev.controller.upsRequired !== null
            ? prev.controller.upsRequired
            : quantities.upsRequiredRecommended,
        networkRequired:
          prev.controller.networkRequired !== undefined && prev.controller.networkRequired !== null
            ? prev.controller.networkRequired
            : true,
      },
    }));
  };
  ...
  useEffect(() => {
    ...
    if (!hasUploadedPlan || editingDoorIndex !== null) return;
    const isDoorDraftEmpty =
      !currentDoor.location &&
      !currentDoor.doorType &&
      (!currentDoor.accessMethod || currentDoor.accessMethod.length === 0) &&
      !currentDoor.lockType &&
      !currentDoor.environment;
    if (!isDoorDraftEmpty) return;
    const used = new Set(data.doors.map((d) => (d.location || '').trim().toLowerCase()).filter(Boolean));
    const nextRoom =
      data.measurements?.rooms?.find((r: any) => !used.has((r?.name || '').trim().toLowerCase())) ||
      data.measurements?.rooms?.[data.doors.length];
    const location = (nextRoom?.name || '').trim() || `Door ${data.doors.length + 1}`;
    setCurrentDoor((prev) => ({
      ...prev,
      name: prev.name || location,
      location: prev.location || location,
      doorType: prev.doorType || 'Single',
      operation: prev.operation || 'Swinging',
      doorAutomation: prev.doorAutomation || 'Manual',
      accessMethod: (prev.accessMethod && prev.accessMethod.length > 0) ? prev.accessMethod : ['RFID Card'],
      accessMethodCapacity: prev.accessMethodCapacity || '100',
      mountingSurface: prev.mountingSurface || 'Wall',
      lockType: prev.lockType || 'Electric strike',
      lockPowerType: prev.lockPowerType || '12V',
      wireType: prev.wireType || ('Low Voltage Power' as any),
      doorMaterial: prev.doorMaterial || 'Metal',
      environment: prev.environment || 'Indoor',
      wallType: prev.wallType || 'Concrete',
    }));
  }, [data.measurements, data.doors, editingDoorIndex, currentDoor]);
                  ...
                  onChange={(m) =>
                    setData(prev => {
                      ...
                      return {
                        ...
                        infrastructure: {
                          ...prev.infrastructure,
                          cableType: prev.infrastructure.cableType || 'Cat6',
                          cablePath: prev.infrastructure.cablePath || 'Ceiling',
                          powerPath: prev.infrastructure.powerPath || 'Separate',
                        },
                        controller: {
                          ...prev.controller,
                          location: prev.controller.location || (firstRoom ? `${firstRoom} - Controller` : 'Security Office'),
                          estimatedCableLength:
                            prev.controller.estimatedCableLength !== undefined
                              ? prev.controller.estimatedCableLength
                              : 30,
                          poeAvailable: prev.controller.poeAvailable === undefined ? true : prev.controller.poeAvailable,
                          redundantControllers:
                            prev.controller.redundantControllers === undefined ? false : prev.controller.redundantControllers,
                          additionalHardware:
                            prev.controller.additionalHardware || 'Controller enclosure, backup battery',
                          wiringNotes: prev.controller.wiringNotes || 'Route via ceiling tray and secure conduit.',
                          powerAvailable: prev.controller.powerAvailable === undefined ? true : prev.controller.powerAvailable,
                          upsRequired: prev.controller.upsRequired === undefined ? true : prev.controller.upsRequired,
                          networkRequired: prev.controller.networkRequired === undefined ? true : prev.controller.networkRequired,
                          fireRatedDoor: prev.controller.fireRatedDoor === undefined ? false : prev.controller.fireRatedDoor,
                        },
                      };
                    })
                  }
        ...
        {step === 'DOORS' && (
          <div className="animate-fade-in space-y-6">
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black text-blue-900 uppercase tracking-tight">Add Access Control Entry</h3>
              </div>
              <div>
        ...
        {step === 'INFRA' && (
          <div className="animate-fade-in space-y-6 pb-12">
            ...
            <div className="space-y-4">
              ...
              {(data.controller.estimatedCableLength == null || data.controller.estimatedCableLength === 30) && (
                <button
                  type="button"
                  onClick={autoFillAccessFromRules}
                  disabled={data.doors.length === 0}
                  className={`w-full py-3 font-black rounded-xl shadow-lg uppercase tracking-widest transition text-[10px] ${
                    data.doors.length === 0 ? 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none' : 'bg-emerald-600 text-white hover:bg-emerald-500'
                  }`}
                >
                  AUTO-FILL CABLE + UPS FROM RULES
                </button>
              )}
