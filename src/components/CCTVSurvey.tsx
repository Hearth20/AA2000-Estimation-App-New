...
import { computeCctvRuleQuantities } from '../utils/materialQuantityRules';
...
interface Props {
  ...
  /** Shared project-level building profile (asked once in Project Details). */
  projectBuildingInfo?: {
    type: string;
    otherType?: string;
    floors: number;
 ...
 */
const CCTVSurvey: React.FC<Props> = ({ onBack, onComplete, onNewFloorPlan, initialData, projectBuildingInfo }) => {
  ...
  const isCameraDraftEmpty = (draft: Partial<CameraEntry>, cableLenText: string, image: string | null) => {
    return !(
      (draft.locationName && draft.locationName.trim()) ||
      (draft.purposes && draft.purposes.length > 0) ||
      draft.type ||
      draft.resolution ||
      draft.lightingCondition ||
      draft.environment ||
      draft.mountingHeight !== undefined ||
      draft.coverageDistanceMeters !== undefined ||
      (draft.scopeStatus && draft.scopeStatus.trim()) ||
      draft.cableType ||
      (draft.otherCableType && draft.otherCableType.trim()) ||
      (cableLenText && cableLenText.trim()) ||
      image
    );
  };

  const buildAutoCameraDraft = (opts?: {
    sourceMeasurements?: BuildingMeasurements;
    sourceCameras?: CameraEntry[];
    sourceBuildingInfo?: CCTVSurveyData['buildingInfo'];
  }): { camera: Partial<CameraEntry>; cableLengthText: string } | null => {
    const sourceMeasurements = opts?.sourceMeasurements ?? measurements;
    const sourceCameras = opts?.sourceCameras ?? cameras;
    const sourceBuildingInfo = opts?.sourceBuildingInfo ?? buildingInfo;
    const rooms = sourceMeasurements?.rooms ?? [];
    const hasUploadedPlan =
      sourceMeasurements?.method === 'PLAN_UPLOAD' &&
      (((sourceMeasurements?.planImages?.length ?? 0) > 0) || !!sourceMeasurements?.planImage);
    if (!rooms.length && !hasUploadedPlan) return null;

    const usedNames = new Set(
      sourceCameras
        .map((c) => (c.locationName || '').trim().toLowerCase())
        .filter(Boolean)
    );
    const nextRoom = rooms.length
      ? (rooms.find((r) => !usedNames.has((r.name || '').trim().toLowerCase())) || rooms[Math.min(sourceCameras.length, rooms.length - 1)])
      : null;
    const roomName = (nextRoom?.name || '').trim() || `Camera Location ${sourceCameras.length + 1}`;

    const area = Number(nextRoom?.area) || 0;
    const coverageDistanceMeters = area > 0 ? Math.max(3, Math.round(Math.sqrt(area))) : (hasUploadedPlan ? 10 : 8);
    const cableLengthMeters = Math.max(10, Math.round(coverageDistanceMeters * 3));

    const primaryType = (CAMERA_TYPES.find((t) => String(t).toLowerCase() === 'dome') || CAMERA_TYPES[0]) as any;
    const primaryResolution = (RESOLUTIONS[0] as any) || undefined;
    const primaryLighting = (LIGHTING_CONDITIONS.find((c) => String(c).toLowerCase().includes('good')) || LIGHTING_CONDITIONS[0]) as any;
    const primaryPurpose = CAMERA_PURPOSES[0];

    return {
      camera: {
        locationName: roomName,
        purposes: primaryPurpose ? [primaryPurpose] : [],
        type: primaryType,
        resolution: primaryResolution,
        lightingCondition: primaryLighting,
        environment: 'Indoor' as any,
        mountingHeight: 2.5,
        coverageDistanceMeters,
        scopeStatus: sourceBuildingInfo.isNew ? 'New Installation' : 'Replacement',
        cableType: 'Cat6' as any,
        otherCableType: '',
      },
      cableLengthText: String(cableLengthMeters),
    };
  };

  const isInfrastructureDraftEmpty = (draft: CCTVSurveyData['infrastructure']) =>
    !draft.cablePath && !draft.wallType && !draft.otherCablePath && !draft.otherWallType && draft.coreDrilling === false;

  const isControlRoomDraftEmpty = (draft: CCTVSurveyData['controlRoom']) =>
    !draft.nvrLocation &&
    draft.storageRequirementTB === undefined &&
    draft.retentionDays === undefined &&
    draft.rackAvailable === undefined &&
    draft.powerSocketAvailable === undefined &&
    draft.upsRequired === undefined &&
    draft.networkSwitchAvailable === undefined &&
    draft.internetAvailable === undefined;

  const buildAutoEquipmentDraft = (opts?: {
    sourceMeasurements?: BuildingMeasurements;
    sourceCameras?: CameraEntry[];
  }): {
    infrastructure: Partial<CCTVSurveyData['infrastructure']>;
    controlRoom: Partial<CCTVSurveyData['controlRoom']>;
  } => {
    const sourceMeasurements = opts?.sourceMeasurements ?? measurements;
    const sourceCameras = opts?.sourceCameras ?? cameras;
    const camCount = sourceCameras.length;
    const totalCable = sourceCameras.reduce((s, c) => s + (Number(c.cableLength) || 0), 0);
    const hasOutdoor = sourceCameras.some((c) => c.environment === 'Outdoor');
    const inferredWall: CCTVSurveyData['infrastructure']['wallType'] = hasOutdoor ? 'Concrete' : 'Gypsum';
    const storageTB = Math.max(2, Math.ceil(camCount / 4) * 2);
    const retentionDays = 30;
    const nvrLocation =
      sourceMeasurements?.rooms?.[0]?.name?.trim()
        ? `${sourceMeasurements.rooms[0].name.trim()} - NVR Area`
        : 'Security Office / Control Room';

    return {
      infrastructure: {
        cablePath: totalCable > 80 ? 'Trunking' : 'Ceiling',
        wallType: inferredWall,
        // `inferredWall` is currently only inferred as 'Concrete' or 'Gypsum'
        // so checking 'Brick' is unreachable under strict TS.
        coreDrilling: hasOutdoor || inferredWall === 'Concrete',
      },
      controlRoom: {
        nvrLocation,
        storageRequirementTB: storageTB,
        retentionDays,
        rackAvailable: true,
  ...
  const autoFillCctvFromRules = () => {
    if (!measurements) return;
    if (cameras.length > 0) return; // avoid overwriting technician-entered inventory

    const quantities = computeCctvRuleQuantities({
      cctvData: {
        buildingInfo,
        cameras: [],
        infrastructure,
    ...
    const desiredCameras = Math.max(0, Math.floor(quantities.camerasRecommended));
    if (desiredCameras <= 0) return;

    const rooms = measurements.rooms ?? [];
    const normalize = (s: unknown) => String(s ?? '').trim().toLowerCase();
    const isCorridorRoom = (name: string) =>
      ['corridor', 'hallway', 'hall ', 'passage', 'corrid'].some((k) => normalize(name).includes(k));
    const isEntryExitRoom = (name: string) =>
      ['entry', 'exit', 'entrance', 'egress', 'lobby', 'drop', 'door'].some((k) => normalize(name).includes(k));

    const corridorNames = rooms.map((r) => r.name).filter((n): n is string => !!n && isCorridorRoom(n));
    const entryExitNames = rooms.map((r) => r.name).filter((n): n is string => !!n && isEntryExitRoom(n));
    ...
    corridorNames.slice(0, desiredCameras).forEach(pushUnique);
    entryExitNames.slice(0, desiredCameras).forEach(pushUnique);

    const otherNames = rooms.map((r) => r.name).filter((n): n is string => !!n && !used.has(n.trim().toLowerCase()));
    let fallbackIdx = 1;
    while (chosenNames.length < desiredCameras) {
      const next = otherNames.length ? otherNames[chosenNames.length % otherNames.length] : '';
      if (next) pushUnique(next);
      else pushUnique(`Camera Location ${fallbackIdx++}`);
    }

    const primaryType = (CAMERA_TYPES.find((t) => String(t).toLowerCase() === 'dome') || CAMERA_TYPES[0]) as any;
    const primaryResolution = (RESOLUTIONS[0] as any) || undefined;
    const primaryLighting =
      (LIGHTING_CONDITIONS.find((c) => String(c).toLowerCase().includes('good')) || LIGHTING_CONDITIONS[0]) as any;
    const primaryPurpose = CAMERA_PURPOSES[0];

    const totalCableMeters = Math.max(0, quantities.cat6CableMetersRecommended);
    const perCameraCable = Math.max(10, Math.round(totalCableMeters / desiredCameras));

    const roomByName = new Map<string, typeof rooms[number]>();
    rooms.forEach((r) => {
      if (!r.name) return;
      roomByName.set(String(r.name).trim().toLowerCase(), r);
    });

    const generated: CameraEntry[] = chosenNames.slice(0, desiredCameras).map((loc) => {
      const room = roomByName.get(loc.trim().toLowerCase());
      const area = Number(room?.area) || 0;
      const coverageDistanceMeters = area > 0 ? Math.max(3, Math.round(Math.sqrt(area))) : 10;
      ...
      return {
        ...
        mountingHeight: 2.5,
        coverageDistanceMeters,
        scopeStatus: buildingInfo.isNew ? 'New Installation' : 'Replacement',
        cableType: 'Cat6' as any,
        otherCableType: '',
        cableLength: perCameraCable,
      };
    });

    setCameras(generated);
  };
  ...
  useEffect(() => {
    if (initialData) {
      const draftBuilding = initialData.buildingInfo;
      setBuildingInfo(draftBuilding);
  ...
  useEffect(() => {
    if (step !== 'CAMERAS' || editingCameraIndex !== null) return;
    if (!isCameraDraftEmpty(currentCamera, cableLengthInput, cameraImage)) return;
    const auto = buildAutoCameraDraft();
    if (!auto) return;
    setCurrentCamera((prev) => ({ ...prev, ...auto.camera }));
    setCableLengthInput((prev) => (prev.trim() ? prev : auto.cableLengthText));
  }, [step, editingCameraIndex, measurements, cameras.length]);

  useEffect(() => {
    if (step !== 'EQUIPMENT') return;
    const infraEmpty = isInfrastructureDraftEmpty(infrastructure);
    const roomEmpty = isControlRoomDraftEmpty(controlRoom);
    if (!infraEmpty && !roomEmpty) return;
    const auto = buildAutoEquipmentDraft();
    if (infraEmpty) {
      setInfrastructure((prev) => ({ ...prev, ...auto.infrastructure }));
    }
    if (roomEmpty) {
      setControlRoom((prev) => ({ ...prev, ...auto.controlRoom }));
    }
  }, [step, cameras, measurements]);
  ...
  const handleHeaderBack = () => {
    if (step === 'EQUIPMENT') {
      ...
      if (hasSharedBuildingInfo) onBack({ buildingInfo, measurements, cameras, infrastructure, controlRoom });
            ...
            <div className="pt-4 border-t border-slate-100">
               ...
               <FloorPlanManager 
                ...
                onChange={(m) => {
                  ...
                  if (!hasUploadedPlan) return;
                  const inferredFloors = Math.max(1, m?.planImages?.length || (m?.planImage ? 1 : 0));
                  setBuildingInfo((prev) => ({
                    ...prev,
                    type: prev.type || 'Office',
                    floors: Number(prev.floors) > 0 ? prev.floors : inferredFloors,
                    isNew: prev.isNew === undefined ? false : prev.isNew,
                  }));
                  if (editingCameraIndex === null && isCameraDraftEmpty(currentCamera, cableLengthInput, cameraImage)) {
                    const autoCam = buildAutoCameraDraft({ sourceMeasurements: m });
                    if (autoCam) {
                      setCurrentCamera((prev) => ({ ...prev, ...autoCam.camera }));
                      setCableLengthInput((prev) => (prev.trim() ? prev : autoCam.cableLengthText));
                    }
                  }
                  const infraEmpty = isInfrastructureDraftEmpty(infrastructure);
                  const roomEmpty = isControlRoomDraftEmpty(controlRoom);
                  if (infraEmpty || roomEmpty) {
                    const autoEq = buildAutoEquipmentDraft({ sourceMeasurements: m });
                    if (infraEmpty) setInfrastructure((prev) => ({ ...prev, ...autoEq.infrastructure }));
                    if (roomEmpty) setControlRoom((prev) => ({ ...prev, ...autoEq.controlRoom }));
                  }
                }}
        ...
        {step === 'CAMERAS' && (
          <div className="animate-fade-in space-y-5">
            <div>
              <button
                type="button"
                disabled={!measurements || cameras.length > 0}
                onClick={autoFillCctvFromRules}
                className={`w-full py-4 font-black rounded-xl shadow-lg uppercase tracking-widest transition text-[10px] ${
                  !measurements || cameras.length > 0 ? 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none' : 'bg-emerald-600 text-white hover:bg-emerald-500'
                }`}
              >
                AUTO-FILL CCTV FROM RULES
              </button>
              {measurements && cameras.length === 0 && (
                <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-2 text-center">
                  Generates camera locations + cable lengths.
                </p>
              )}
            </div>
                    ...
                    <div className="relative mt-1">
                      <input 
                        ...
                        onChange={e => {
                          ...
                          if (raw === '') {
                            setCurrentCamera({ ...currentCamera, mountingHeight: undefined });
                            return;
                          }
                          const normalized = raw.replace(/[^\d.]/g, '');
                          const numeric = Number(normalized);
                          if (Number.isNaN(numeric)) return;
                          setCurrentCamera({ ...currentCamera, mountingHeight: numeric });
                        }}
                      />
