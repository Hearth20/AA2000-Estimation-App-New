...
const FireProtectionSurvey: React.FC<Props> = ({ onBack, onComplete, onNewFloorPlan, initialData, projectBuildingInfo }) => {
  ...
  // Auto-calc sprinkler pipe length from coverage area + hazard classification.
  // Keeps the app consistent with the rule-of-thumb: heads ~= area/9 (Light) or area/6 (Ordinary), then pipeLen ~= heads*3.
  useEffect(() => {
    const hasSprinkler = Array.isArray(data.scope.systems) && data.scope.systems.includes('Sprinkler');
    if (!hasSprinkler) return;
    if (!data.sprinkler.coverageArea || Number(data.sprinkler.coverageArea) <= 0) return;
    if (Number(data.sprinkler.pipeLength) > 0) return; // don't overwrite technician input

    const area = Number(data.sprinkler.coverageArea) || 0;
    const hazard = String(data.hazardClassification || '').toLowerCase();
    const sqmPerHead = hazard.includes('light') ? 9 : 6; // Extra hazard treated as ordinary in this app
    const heads = Math.max(1, Math.ceil(area / sqmPerHead));
    const pipeLengthM = heads * 3;

    setData((prev) => ({
      ...prev,
      sprinkler: {
        ...prev.sprinkler,
        pipeLength: pipeLengthM,
      },
    }));
  }, [data.scope.systems, data.sprinkler.coverageArea, data.sprinkler.pipeLength, data.hazardClassification]);
                ...
                <FloorPlanManager 
                  ...
                  onChange={(m) =>
                    setData(prev => {
                      ...
                      const inferredArea =
                        Number(prev.buildingInfo.area) > 0
                          ? prev.buildingInfo.area
                          : Math.max(
                              100,
                              Math.round(
                                (m?.rooms || []).reduce((sum: number, r: any) => sum + (Number(r?.area) || 0), 0)
                              )
                            );
                      return {
                        ...prev,
                        ...
                        protectionArea: prev.protectionArea || 'Office',
                        hazardClassification: prev.hazardClassification || 'Ordinary Hazard',
                        scope: {
                          ...prev.scope,
                          systems: prev.scope.systems.length ? prev.scope.systems : ['Suppression'],
                          status: prev.scope.status || (prev.buildingInfo.isNew ? 'New Installation' : 'Expansion'),
                        },
                        alarmCore: {
                          ...prev.alarmCore,
                          type: prev.alarmCore.type || 'Addressable',
                          panelLocation: prev.alarmCore.panelLocation || (firstRoom ? `${firstRoom} - FACP` : 'Control Room'),
                          powerAvailable: prev.alarmCore.powerAvailable === undefined ? true : prev.alarmCore.powerAvailable,
                          batteryRequired: prev.alarmCore.batteryRequired === undefined ? true : prev.alarmCore.batteryRequired,
                          smokeCount: Number(prev.alarmCore.smokeCount) > 0 ? prev.alarmCore.smokeCount : 2,
                          heatCount: Number(prev.alarmCore.heatCount) > 0 ? prev.alarmCore.heatCount : 1,
                          mcpCount: Number(prev.alarmCore.mcpCount) > 0 ? prev.alarmCore.mcpCount : 1,
                          notifCount: Number(prev.alarmCore.notifCount) > 0 ? prev.alarmCore.notifCount : 1,
                        },
                        zoning: {
                          ...prev.zoning,
                          zones: Number(prev.zoning.zones) > 0 ? prev.zoning.zones : Math.max(1, inferredFloors),
                        },
                        infrastructure: {
                          ...prev.infrastructure,
                          cableType: prev.infrastructure.cableType || 'Fire-rated',
                          cableLength: Number(prev.infrastructure.cableLength) > 0 ? prev.infrastructure.cableLength : 50,
                          conduitsExist: prev.infrastructure.conduitsExist === undefined ? true : prev.infrastructure.conduitsExist,
                        },
                        suppression: {
                          ...prev.suppression,
                          type: prev.suppression.type || 'ABC',
                          qty: Number(prev.suppression.qty) > 0 ? prev.suppression.qty : 1,
                          locationIdentified:
                            prev.suppression.locationIdentified === undefined ? true : prev.suppression.locationIdentified,
                          coverageType: prev.suppression.coverageType || 'Localized',
                          cylinderLocAvailable:
                            prev.suppression.cylinderLocAvailable === undefined ? true : prev.suppression.cylinderLocAvailable,
                          nozzleCount: Number(prev.suppression.nozzleCount) > 0 ? prev.suppression.nozzleCount : 1,
                          sealingCondition: prev.suppression.sealingCondition || 'Good',
                        },
                        sprinkler: {
                          ...prev.sprinkler,
                          coverageArea: Number(prev.sprinkler.coverageArea) > 0 ? prev.sprinkler.coverageArea : inferredArea,
                          waterSource: prev.sprinkler.waterSource || 'Tank',
                          pumpRoomAvailable:
                            prev.sprinkler.pumpRoomAvailable === undefined ? true : prev.sprinkler.pumpRoomAvailable,
                          existingStatus: prev.sprinkler.existingStatus || 'New',
                          headType: prev.sprinkler.headType || 'Pendant',
                          tempRating: prev.sprinkler.tempRating || '68C',
                          pipeMaterial: prev.sprinkler.pipeMaterial || 'GI',
                          pipeRouting: prev.sprinkler.pipeRouting || 'Ceiling',
                          pipeLength: Number(prev.sprinkler.pipeLength) > 0 ? prev.sprinkler.pipeLength : 30,
                        },
                        integration: {
                          ...prev.integration,
                          bfpCompliance: prev.integration.bfpCompliance === undefined ? true : prev.integration.bfpCompliance,
                        },
                        siteConstraints: {
                          ...prev.siteConstraints,
                          ceilingHeight: Number(prev.siteConstraints.ceilingHeight) > 0 ? prev.siteConstraints.ceilingHeight : 3,
                          ceilingType: prev.siteConstraints.ceilingType || 'Concrete',
                          isOccupied: prev.siteConstraints.isOccupied === undefined ? false : prev.siteConstraints.isOccupied,
                        },
                        controlRoom: {
                          ...prev.controlRoom,
                          name: prev.controlRoom?.name || (firstRoom ? `${firstRoom} Control Room` : 'Control Room'),
                          floorLevel: prev.controlRoom?.floorLevel || 'Ground Floor',
                          distanceToArea:
                            Number(prev.controlRoom?.distanceToArea) > 0 ? prev.controlRoom.distanceToArea : 20,
                          panelInstalled:
                            prev.controlRoom?.panelInstalled === undefined ? 'No' : prev.controlRoom.panelInstalled,
                          panelType:
                            prev.controlRoom?.panelType === undefined
                              ? 'Dedicated suppression panel'
                              : prev.controlRoom.panelType,
                          releaseMethod:
                            prev.controlRoom?.releaseMethod === undefined ? 'Combined' : prev.controlRoom.releaseMethod,
                          powerSupplyAvailable:
                            prev.controlRoom?.powerSupplyAvailable === undefined ? true : prev.controlRoom.powerSupplyAvailable,
                          upsBackupProvided:
                            prev.controlRoom?.upsBackupProvided === undefined ? true : prev.controlRoom.upsBackupProvided,
                        },
                        fireExtinguisher: {
                          ...(prev.fireExtinguisher || { type: '', quantity: 0, capacity: '', mountingType: '', otherType: '', otherCapacity: '' }),
                          type: prev.fireExtinguisher?.type || 'ABC',
                          quantity: Number(prev.fireExtinguisher?.quantity) > 0 ? prev.fireExtinguisher!.quantity : 1,
                          capacity: prev.fireExtinguisher?.capacity || '5 kg',
                          mountingType: prev.fireExtinguisher?.mountingType || 'Wall-mounted',
                        },
                        fireHoseReel: {
                          ...(prev.fireHoseReel || { quantity: 0, hoseLengthM: 0, nozzleType: '', otherNozzleType: '' }),
                          quantity: Number(prev.fireHoseReel?.quantity) > 0 ? prev.fireHoseReel!.quantity : 1,
                          hoseLengthM: Number(prev.fireHoseReel?.hoseLengthM) > 0 ? prev.fireHoseReel!.hoseLengthM : 30,
                          nozzleType: prev.fireHoseReel?.nozzleType || 'Jet/Spray',
                        },
                        fireBlanket: {
                          ...(prev.fireBlanket || { quantity: 0, locations: '' }),
                          quantity: Number(prev.fireBlanket?.quantity) > 0 ? prev.fireBlanket!.quantity : 1,
                          locations: prev.fireBlanket?.locations || 'Kitchen Area',
                        },
                        emergencyLighting: {
                          ...(prev.emergencyLighting || { present: false, type: '' }),
                          present: prev.emergencyLighting?.present ?? true,
                          type: prev.emergencyLighting?.type || 'Maintained',
                        },
                        exitEvacuation: {
                          ...(prev.exitEvacuation || { exitSignsQuantity: 0, evacuationLightingPresent: false }),
                          exitSignsQuantity:
                            Number(prev.exitEvacuation?.exitSignsQuantity) > 0 ? prev.exitEvacuation!.exitSignsQuantity : 2,
                          evacuationLightingPresent:
                            prev.exitEvacuation?.evacuationLightingPresent ?? true,
                        },
                      };
                    })
                  }
        ...
        {step === 'DETAILS' && (
          <div className="animate-fade-in space-y-6">
            ...
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-6">
               <div className="flex justify-between items-center">
                  <h3 className="text-xl font-black text-blue-900 uppercase tracking-tight">Add Fire Protection Entry</h3>
               </div>
