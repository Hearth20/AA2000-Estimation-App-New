...
import { Project, SurveyType, EstimationDetail, EstimationManpowerEntry, EstimationConsumableEntry, EstimationAdditionalFeeEntry, CCTVSurveyData, FireAlarmSurveyData, AccessControlSurveyData, BurglarAlarmSurveyData, FireProtectionSurveyData, OtherSurveyData, BuildingMeasurements } from '../types';
import axios from 'axios';
import { createEstimationDocx } from './BOQ';
...
import { projectSurveyScope, technicianSurveyTasks } from '../utils/projectSurveyVisibility';
...
const EstimationScreen: React.FC<Props> = ({ 
  project, viewerRole, viewerEmail, type, cctvData, faData, fpData, acData, baData, otherData,
  ...
  const [hasAutoEstimatedFromPlan, setHasAutoEstimatedFromPlan] = useState(false);
  ...
  const uploadEndpoint = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_ESTIMATION_UPLOAD_ENDPOINT)
    ? (import.meta.env.VITE_ESTIMATION_UPLOAD_ENDPOINT as string).trim()
    : '';
  const uploadPath = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_ESTIMATION_UPLOAD_PATH)
    ? (import.meta.env.VITE_ESTIMATION_UPLOAD_PATH as string).trim()
    : '';
  ...
  const uploadEstimationToApi = async (est: EstimationDetail) => {
    ...
    const normalizedEndpoint = uploadEndpoint
      ? (uploadEndpoint.startsWith('http')
          ? uploadEndpoint
          : `/${uploadEndpoint.replace(/^\/+/, '')}`)
      : '';
    // Prefer relative URLs so dev requests pass through Vite proxy and avoid CORS.
    ...
    const fileFieldCandidates = ['estimationDoc', 'file'];
      ...
      for (const fileField of fileFieldCandidates) {
        ...
        try {
          ...
          if (axios.isAxiosError(err)) {
            ...
            if (status === 404 || status === 405 || status === 400 || status === 415) {
              continue;
            }
          }
        }
  ...
  const allowedSurveyTypes =
    viewerRole === 'TECHNICIAN'
      ? technicianSurveyTasks(project, viewerEmail || '')
      : projectSurveyScope(project);

  const currentMeasurements: BuildingMeasurements | undefined = (() => {
    if (type === SurveyType.CCTV) return cctvData?.measurements;
    if (type === SurveyType.FIRE_ALARM) return faData?.measurements;
    if (type === SurveyType.FIRE_PROTECTION) return fpData?.measurements;
    if (type === SurveyType.ACCESS_CONTROL) return acData?.measurements;
    if (type === SurveyType.BURGLAR_ALARM) return baData?.measurements;
    if (type === SurveyType.OTHER) return otherData?.measurements;
    return undefined;
  })();
  const hasFloorPlanUpload =
    currentMeasurements?.method === 'PLAN_UPLOAD' &&
    (((currentMeasurements.planImages?.length ?? 0) > 0) || !!currentMeasurements.planImage);
  const planAreaSqm = Math.max(0, Number(currentMeasurements?.totalArea) || 0);
  const planRoomsCount = currentMeasurements?.rooms?.length || 0;
  const planPagesCount =
    (currentMeasurements?.planImages?.length ?? 0) ||
    (currentMeasurements?.planImage ? 1 : 0);
  /** When a plan is uploaded but AI/manual did not yield room areas yet, totalArea is 0 — still estimate consumables from pages / room names. */
  const effectivePlanAreaSqm = hasFloorPlanUpload
    ? Math.max(planAreaSqm, planRoomsCount * 12, planPagesCount * 28, 25)
    : planAreaSqm;

  const availableSurveys = Object.values(SurveyType).filter(s => {
    if (!allowedSurveyTypes.includes(s)) return false;
  ...
  const addOrReplaceConsumables = (entries: ConsumableEntry[]) => {
    setConsumablesList((prev) => {
      const next = [...prev];
      entries.forEach((entry) => {
        const existingIdx = next.findIndex((x) => x.name === entry.name);
        if (existingIdx >= 0) {
          next[existingIdx] = { ...next[existingIdx], qty: entry.qty, category: entry.category, unitPrice: entry.unitPrice };
        } else {
          next.push(entry);
        }
      });
  ...
  const buildFloorPlanConsumableEstimate = (): ConsumableEntry[] => {
    if (!hasFloorPlanUpload || effectivePlanAreaSqm <= 0) return [];
    const areaUnits = Math.max(1, Math.ceil(effectivePlanAreaSqm / 25)); // 1 unit per 25 sqm
    const routingUnits = Math.max(1, Math.ceil(effectivePlanAreaSqm / 40)); // lighter density for routing consumables
    const roomUnits = Math.max(1, planRoomsCount || 1);
    const add = (name: string, category: string, qty: number): ConsumableEntry => ({
      id: `auto-${Date.now()}-${name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`,
      name,
      category,
    ...
    let generated: ConsumableEntry[] = [];
    if (type === SurveyType.CCTV) {
      generated = [
        add('Self-tapping screws', 'Fastening', areaUnits * 42),
        add('Wall anchors', 'Fastening', areaUnits * 30),
        add('Electrical tape', 'Electrical & Finishing', routingUnits * 3),
        add('Cable markers / labels', 'Electrical & Finishing', areaUnits * 3),
        add('Silicone sealant (weatherproof)', 'Sealing & Protection', routingUnits * 1.5),
        add('Cleaning wipes', 'Electrical & Finishing', Math.ceil(roomUnits / 2)),
      ];
    } else if (type === SurveyType.FIRE_ALARM) {
      generated = [
        add('Self-tapping screws', 'Fastening', areaUnits * 50),
        add('Wall anchors', 'Fastening', areaUnits * 35),
        add('Fire-rated silicone sealant', 'Sealing & Protection', routingUnits * 2),
        add('Firestop putty', 'Sealing & Protection', routingUnits * 2),
        add('Electrical tape', 'Electrical & Finishing', routingUnits * 4),
        add('Zone labels', 'Electrical & Finishing', Math.max(roomUnits, areaUnits * 2)),
      ];
    } else if (type === SurveyType.ACCESS_CONTROL) {
      generated = [
        add('Self-tapping screws', 'Fastening', areaUnits * 36),
        add('Expansion anchors', 'Fastening', areaUnits * 22),
        add('Wire ferrules', 'Electrical & Finishing', areaUnits * 16),
        add('Terminal lugs', 'Electrical & Finishing', areaUnits * 12),
        add('Cable markers / labels', 'Electrical & Finishing', areaUnits * 2),
        add('Electrical tape', 'Electrical & Finishing', routingUnits * 3),
      ];
    } else if (type === SurveyType.BURGLAR_ALARM) {
      generated = [
        add('Self-tapping screws', 'Fastening', areaUnits * 34),
        add('Wall plugs / Rawl plugs', 'Fastening', areaUnits * 24),
        add('Electrical tape', 'Electrical & Finishing', routingUnits * 3),
        add('Cable markers / labels', 'Electrical & Finishing', areaUnits * 2),
        add('Silicone sealant (weatherproof)', 'Sealing & Protection', routingUnits),
        add('Cleaning wipes', 'Electrical & Finishing', Math.ceil(roomUnits / 2)),
      ];
    } else if (type === SurveyType.FIRE_PROTECTION) {
      generated = [
        add('Expansion bolts', 'Fastening', areaUnits * 28),
        add('Anchor bolts', 'Fastening', areaUnits * 18),
        add('Teflon tape', 'Sealing & Protection', routingUnits * 4),
        add('Pipe joint compound', 'Sealing & Protection', routingUnits * 3),
        add('Firestop sealant', 'Sealing & Protection', routingUnits * 2),
        add('Warning stickers', 'Electrical & Finishing', Math.max(roomUnits, areaUnits * 2)),
      ];
    } else {
      generated = [
        add('Self-tapping screws', 'Fastening', areaUnits * 35),
        add('Wall anchors', 'Fastening', areaUnits * 25),
        add('Electrical tape', 'Electrical & Finishing', routingUnits * 2),
        add('Cable markers / labels', 'Electrical & Finishing', areaUnits * 2),
        add('Silicone sealant (weatherproof)', 'Sealing & Protection', routingUnits),
        add('Cleaning wipes', 'Electrical & Finishing', Math.ceil(roomUnits / 2)),
      ];
  ...
  const estimateMaterialsFromFloorPlan = () => {
    const generated = buildFloorPlanConsumableEstimate();
    if (generated.length === 0) return;
    addOrReplaceConsumables(generated);
    setHasAutoEstimatedFromPlan(true);
  };

  useEffect(() => {
    // Auto-run once when floor-plan measurements are available and no consumables exist yet.
    // This avoids overriding manual technician edits after the initial estimate.
    if (!hasFloorPlanUpload || effectivePlanAreaSqm <= 0) return;
    if (consumablesList.length > 0) return;
    const generated = buildFloorPlanConsumableEstimate();
    if (generated.length === 0) return;
    addOrReplaceConsumables(generated);
    setHasAutoEstimatedFromPlan(true);
  }, [hasFloorPlanUpload, effectivePlanAreaSqm, planRoomsCount, planPagesCount, type]);
        ...
        <div className="border-2 border-slate-200 rounded-[1.5rem] overflow-hidden bg-white shadow-sm">
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-[10px] font-black text-blue-900 uppercase tracking-widest ml-2">Consumables</h3>
              {hasFloorPlanUpload && (
                <button
                  type="button"
                  onClick={estimateMaterialsFromFloorPlan}
                  className="px-3 py-2 border-2 border-emerald-600 rounded-xl text-[9px] font-black text-emerald-700 uppercase tracking-widest hover:bg-emerald-50 transition-colors"
                >
            ...
            {hasFloorPlanUpload && (
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">
                Plan detected:{' '}
                {planAreaSqm > 0
                  ? `${planAreaSqm.toFixed(1)} sqm`
                  : `~${effectivePlanAreaSqm.toFixed(0)} sqm (estimated from ${planPagesCount} plan page${planPagesCount === 1 ? '' : 's'} until room areas are known)`}{' '}
                across {planRoomsCount || 0} room{planRoomsCount === 1 ? '' : 's'}.
              </p>
            )}
            {hasAutoEstimatedFromPlan && (
              <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wide">
                Auto-estimated from floor plan. You can still edit quantities.
              </p>
            )}
            ...
            <h3 className="text-[10px] font-black text-blue-900 uppercase tracking-widest ml-2">Phase Effort Details</h3>
        ...
        <div className="pt-6 space-y-3 pb-6">
          <button
            ...
            onClick={async () => {
              ...
              // Technicians must still be able to save local technical breakdown even if upload endpoint is down.
              if (uploadFailed && canViewCosting) {
                return;
              }
              onComplete(est);
            }}
