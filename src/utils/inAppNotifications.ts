...
export interface InAppNotification {
  ...
  message: string;
  /** Saved project row key; used for in-app navigation. */
  projectId: string;
  projectName?: string;
  /** When provided by the server, ties the row to a finalization audit entry. */
  approvalAuditEntryId?: string;
...
export function getVisibleNotifications(userEmail: string, userRole: 'TECHNICIAN' | 'ADMIN' | null): InAppNotification[] {
  ...
  return readAll()
    .filter((n) => isVisible(n, userEmail, userRole, prefs) && !n.read)
...
/** Mark one notification read (e.g. after opening its target). */
